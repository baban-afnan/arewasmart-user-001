...
class NINPhoneVerificationController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $service = ServiceManager::getServiceWithFields('Verification', [
            ...
            ['name' => 'VNIN Slip', 'code' => '613', 'price' => 100],
        ]);
        ...
        if ($service) {
            ...
            $vninField = $service->fields->where('field_code', '613')->first();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            \Log::error('NIN Phone Verification Error', [
    ...
    private function processSuccessTransaction($wallet, $servicePrice, $user, $serviceField, $service, $apiResponse)
    {
        ...
        try {
            ...
            $ninData = is_array($dataArray) && !empty($dataArray) ? (isset($dataArray[0]) ? $dataArray[0] : $dataArray) : [];
            ...
            foreach ($criticalFields as $field) {
                if (isset($ninData[$field]) && str_contains((string)$ninData[$field], '****')) {
                    $isSuspended = true;
            ...
            if ($isSuspended) {
                \Log::warning('NIN Phone Verification - Masked Data Detected', [
                    'user_id' => $user->id,
            ...
            $transactionRef = 'Pa' . (time() % 10000000) . '-' . mt_rand(10, 99);
            ...
            $transaction = Transaction::create([
                ...
                'user_id' => $user->id,
                'amount' => $servicePrice,
                'description' => "NIN Phone Verification: " . ($apiResponse['value'] ?? 'N/A'),
                'type' => 'debit',
                'status' => 'completed',
            ...
            // Map fields to verification table
            Verification::create([
                'user_id' => $user->id,
            ...
            $message = "NIN Phone Verification successful. Charged: NGN " . number_format($servicePrice, 2);
            if ($isSuspended) {
                $message .= " | WARNING: This NIN appears to be suspended or restricted (masked data detected).";
            }
            ...
            DB::rollBack();
            \Log::error('NIN Phone Verification Transaction Error', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
            return back()->with([
    ...
    private function chargeForSlip($user, $fieldCode)
    {
        ...
        DB::beginTransaction();
        try {
             $transactionRef = 'SLIP-' . (time() % 10000000) . '-' . mt_rand(10, 99);
             $performedBy = $user->first_name . ' ' . $user->last_name;
