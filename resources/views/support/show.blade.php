<x-app-layout>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="card shadow-sm border-0" style="height: 75vh; display: flex; flex-direction: column;">
                        ...
                        <!-- Chat Messages Area -->
                        <div class="card-body bg-white overflow-auto p-4" id="messages-area" style="flex: 1; background-color: #f8f9fa;">
                            @if(session('success'))
                            ...
                            @foreach($ticket->messages as $message)
                                <div class="d-flex mb-4 {{ $message->is_admin_reply ? 'justify-content-start' : 'justify-content-end' }}" data-id="{{ $message->id }}">
    ...
    // Scroll after new messages are loaded
    function reloadMessages() {
        const messagesArea = document.getElementById('messages-area');
        ...
        fetch(`{{ route('support.updates', $ticket->ticket_reference) }}?last_message_id=${lastMessageId}`)
            ...
            .then(data => {
                ...
                scrollToLastMessage(); // Scroll after reload
            })
            .catch(console.error);
    }
